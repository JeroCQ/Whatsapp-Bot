...
class Settings:
    ...
    # Stay below Railway's proxy deadline so asynchronous jobs retain a useful error.
    DASHBOARD_FORMAT_TIMEOUT_SECONDS = float(os.getenv("DASHBOARD_FORMAT_TIMEOUT_SECONDS", "45"))
    DASHBOARD_STORAGE_TIMEOUT_SECONDS = float(os.getenv("DASHBOARD_STORAGE_TIMEOUT_SECONDS", "300"))
    ...
    def validate(cls):
        ...
        if any(chatwoot_values):
            ...
            if cls.CHATWOOT_ASSIGNMENT_MODE not in {"automatic", "fixed"}:
                raise ValueError("CHATWOOT_ASSIGNMENT_MODE must be 'automatic' or 'fixed'")
            ...
            if cls.CHATWOOT_ASSIGNMENT_MODE == "fixed" and not cls.CHATWOOT_ASSIGNEE_ID:
                raise ValueError("CHATWOOT_ASSIGNEE_ID is required when CHATWOOT_ASSIGNMENT_MODE=fixed")
